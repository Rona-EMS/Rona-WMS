...
import { ShieldCheck, ArrowLeft, Globe, Check, ChevronDown, ImageIcon } from "lucide-react"
...
export default function AdminLoginPage() {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await login(email || "admin@rona.com", password || "password", "admin")

      if (success) {
        toast({
          title: t("common.success"),
          description: language === "en" ? "Redirecting to admin dashboard..." : "ወደ አስተዳዳሪ ዳሽቦርድ በማዘዋወር ላይ...",
          duration: 3000,
        })
      }
    } catch (error) {
  ...
  const handleOtpLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (otpToVerify.length === 6 || otpToVerify === generatedOtp) {
        ...
        const success = await login(email || "admin@rona.com", `otp-auth-${otpToVerify}`, "admin")
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-purple-100 via-purple-50 to-white overflow-hidden">
      {/* Decorative background elements */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        {/* Large circle in bottom right */}
        <div className="absolute -bottom-20 -right-20 w-96 h-96 rounded-full bg-purple-100/70 blur-sm"></div>

        {/* Small circle in top left */}
        <div className="absolute -top-10 -left-10 w-72 h-72 rounded-full bg-purple-100/70 blur-sm"></div>
      </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-10 flex justify-between items-center p-4">
        <ConsistentClock variant="header" textColor="var(--purple-600)" />
      ...
      <div className="flex flex-col items-center justify-center p-4 min-h-screen relative z-10">
        <div className="w-full max-w-4xl mb-6 flex flex-col items-center justify-center text-center">
          <div className="flex flex-col items-center justify-center mb-2">
            <div className="w-16 h-16 bg-purple-50 rounded-full flex items-center justify-center border border-purple-200 mb-2">
              <ImageIcon className="w-8 h-8 text-purple-400" />
            </div>
            <p className="text-purple-600 text-sm font-medium">Your company logo</p>
          </div>
        </div>

        <Card className="w-full max-w-md shadow-sm backdrop-blur-sm bg-white/90 border-purple-200">
          <CardHeader className="pb-2 pt-2 text-center">
            <div className="flex flex-col items-center mb-2">
              <div className="p-2 rounded-full mb-2 bg-purple-100">
                <ShieldCheck className="h-4 w-4" />
              </div>
              <div>
                <CardTitle className="text-xl">
                  {language === "en" ? "Admin App Login" : "የአስተዳዳሪ መተግበሪያ መግቢያ"}
                </CardTitle>
                <CardDescription className="text-xs">
                  {language === "en"
                    ? "Enter your credentials to access the admin dashboard"
                    : "የእርስዎን ማረጋገጫዎች ያስገቡ ወደ አስተዳዳሪ ዳሽቦርድ ለመድረስ"}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <TabsContent value="password">
              <form onSubmit={handlePasswordLogin}>
                <CardContent className="space-y-3 pt-4">
                  <div className="space-y-1">
                    ...
                    <Input
                      id="email"
                      type="email"
                      placeholder={language === "en" ? "admin@rona.com" : "አስተዳዳሪ@rona.com"}
                      value={email}
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="text-xs underline-offset-4 hover:underline text-purple-600"
                      >
                ...
                <CardFooter className="flex flex-col space-y-3 pt-2">
                  <Button
                    type="submit"
                    className="w-4/5 mx-auto h-9 text-white bg-purple-600 hover:bg-purple-700"
                  ...
                  <div className="text-center w-full">
                    ...
                    <Link href="/register" className="text-xs underline-offset-4 hover:underline text-purple-600">
                      {t("login.register")}
                    </Link>
                  </div>
                </CardFooter>
            ...
            <TabsContent value="otp">
              <form onSubmit={handleOtpLogin}>
                <CardContent className="space-y-3 pt-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={sendOtp}
                        className="h-9 text-xs border-purple-600 text-purple-600 hover:bg-purple-50"
                      >
                        {t("login.sendOtp")}
                      </Button>
                    </div>
                  ...
                  {generatedOtp && (
                    <div className="mt-1 p-2 bg-purple-100 rounded-md border border-purple-200 text-center">
                      <p className="text-[10px] text-purple-700 mb-1">{t("login.yourOtpCode")}</p>
                      <p className="text-lg font-mono font-bold tracking-widest text-purple-700">{generatedOtp}</p>
                    </div>
                  )}
                </CardContent>
                <CardFooter className="flex flex-col space-y-3 pt-2">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex justify-center gap-2">
                <Link href="/worker-login">
                  ...
                  >
                    {language === "en" ? "Worker App" : "የሰራተኛ መተግበሪያ"}
                  </Button>
                </Link>
