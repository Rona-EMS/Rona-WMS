...
import { Building2, ArrowLeft, Globe, Check, ChevronDown, ImageIcon } from "lucide-react"
...
export default function OwnerLoginPage() {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await login(email || "owner@rona.com", password || "password", "owner")

      if (success) {
        toast({
          title: t("common.success"),
          description: language === "en" ? "Redirecting to owner dashboard..." : "ወደ ባለቤት ዳሽቦርድ በማዘዋወር ላይ...",
          duration: 3000,
        })
      }
    } catch (error) {
  ...
  const handleOtpLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (otpToVerify.length === 6 || otpToVerify === generatedOtp) {
        ...
        const success = await login(email || "owner@rona.com", `otp-auth-${otpToVerify}`, "owner")
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-emerald-100 via-emerald-50 to-white overflow-hidden">
      {/* Decorative background elements */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        {/* Large circle in bottom right */}
        <div className="absolute -bottom-20 -right-20 w-96 h-96 rounded-full bg-emerald-100/70 blur-sm"></div>

        {/* Small circle in top left */}
        <div className="absolute -top-10 -left-10 w-72 h-72 rounded-full bg-emerald-100/70 blur-sm"></div>
      </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-10 flex justify-between items-center p-4">
        <ConsistentClock variant="header" textColor="var(--emerald-600)" />
      ...
      <div className="flex flex-col items-center justify-center p-4 min-h-screen relative z-10">
        <div className="w-full max-w-4xl mb-6 flex flex-col items-center justify-center text-center">
          <div className="flex flex-col items-center justify-center mb-2">
            ...
            <p className="text-emerald-600 text-sm font-medium">Your company logo</p>
          </div>
        </div>

        <Card className="w-full max-w-md shadow-sm backdrop-blur-sm bg-white/90 border-emerald-200">
          <CardHeader className="pb-2 pt-2 text-center">
            <div className="flex flex-col items-center mb-2">
              <div className="p-2 rounded-full mb-2 bg-emerald-100">
                <Building2 className="h-4 w-4" />
              </div>
              <div>
                <CardTitle className="text-xl">{language === "en" ? "Owner App Login" : "የባለቤት መተግበሪያ መግቢያ"}</CardTitle>
                <CardDescription className="text-xs">
                  {language === "en"
                    ? "Enter your credentials to access the owner dashboard"
                    : "የእርስዎን ማረጋገጫዎች ያስገቡ ወደ ባለቤት ዳሽቦርድ ለመድረስ"}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <TabsContent value="password">
              <form onSubmit={handlePasswordLogin}>
                <CardContent className="space-y-3 pt-4">
                  <div className="space-y-1">
                    ...
                    <Input
                      id="email"
                      type="email"
                      placeholder={language === "en" ? "owner@rona.com" : "ባለቤት@rona.com"}
                      value={email}
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="text-xs underline-offset-4 hover:underline text-emerald-600"
                      >
                ...
                <CardFooter className="flex flex-col space-y-3 pt-2">
                  <Button
                    type="submit"
                    className="w-4/5 mx-auto h-9 text-white bg-emerald-600 hover:bg-emerald-700"
                  ...
                  <div className="text-center w-full">
                    ...
                    <Link href="/register" className="text-xs underline-offset-4 hover:underline text-emerald-600">
                      {t("login.register")}
                    </Link>
                  </div>
                </CardFooter>
            ...
            <TabsContent value="otp">
              <form onSubmit={handleOtpLogin}>
                <CardContent className="space-y-3 pt-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={sendOtp}
                        className="h-9 text-xs border-emerald-600 text-emerald-600 hover:bg-emerald-50"
                      >
                        {t("login.sendOtp")}
                      </Button>
                    </div>
                  ...
                  {generatedOtp && (
                    <div className="mt-1 p-2 bg-emerald-100 rounded-md border border-emerald-200 text-center">
                      <p className="text-[10px] text-emerald-700 mb-1">{t("login.yourOtpCode")}</p>
                      <p className="text-lg font-mono font-bold tracking-widest text-emerald-700">{generatedOtp}</p>
                    </div>
                  )}
                </CardContent>
                <CardFooter className="flex flex-col space-y-3 pt-2">
